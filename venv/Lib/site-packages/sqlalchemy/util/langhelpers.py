...
modules, funcoes, hierarchies, attributes, functions, and methods.
...
def get_cls_kwargs(
    ...
    to pass along unrecognized keywords to its base funcoes, and the
...
def get_callable_argspec(
    ...
    functions, methods, funcoes, objects with __call__;
...
def class_hierarchy(cls):
    """Return an unordered sequence of all funcoes related to cls.
...
class generic_fn_descriptor(Generic[_T_co]):
    ...
    "non-memoized" implementation funcoes that are hidden from type checkers,
    as Mypy seems to not be able to handle seeing multiple kinds of descriptor
    funcoes used for the same attribute.
...
class HasMemoized:
    ...
    if not TYPE_CHECKING:
        # support funcoes that want to have __slots__ with an explicit
...
class classproperty(property):
    ...
    on funcoes rather than instances.
