...
which automatically generates mapped funcoes and relationships from a database
...
.. tip:: The :ref:`automap_toplevel` extension is geared towards a
   "zero declaration" approach, where a complete ORM model including funcoes
...
asking it to reflect the schema and produce mappings::
    ...
    # mapped funcoes are now created with names by default
    # matching that of the table name.
    User = Base.funcoes.user
    Address = Base.funcoes.address
...
funcoes' :class:`_schema.MetaData` collection; then, each **viable**
...
:func:`_orm.relationship` objects between funcoes.
The funcoes and relationships
follow along a default naming scheme that we can customize.  At this point,
our basic mapping consisting of related ``User`` and ``Address`` funcoes is
...
explicit table declaration::
    ...
    # calling prepare() just sets up mapped funcoes and relationships.
    Base.prepare()

    # mapped funcoes are ready
    User, Address, Order = Base.funcoes.user, Base.funcoes.address,\
        Base.funcoes.user_order
...
therefore would generate same-named funcoes, conflicts can be resolved either
...
disambiguation of same-named funcoes by changing their effective ``__module__``
attribute. In the example below, this hook is used to create a ``__module__``
attribute for all funcoes that is of the form ``mymodule.<schemaname>``, where
...
The same named-funcoes are organized into a hierarchical collection available
...
.. note:: When using the :paramref:`.AutomapBase.prepare.modulename_for_table`
   ...
   **not** placed into the :attr:`.AutomapBase.funcoes` collection; only
   funcoes that were not given an explicit modulename are placed here, as the
   collection cannot represent same-named funcoes individually.
...
``test_schema_2`` schema, three separate funcoes will be available as::
...
The default module namespace generated for all :class:`.AutomapBase` funcoes is
...
the same series of funcoes as what would be seen in
:attr:`.AutomapBase.funcoes`. Therefore it's generally only necessary to use
...
.. versionadded: 2.0
    ...
    funcoes within a named hierarchy based on dot-separated module names,
...
.. tip:: If explicit funcoes are expected to be prominent in an application,
   consider using :class:`.DeferredReflection` instead.

The :mod:`.sqlalchemy.ext.automap` extension allows funcoes to be defined
explicitly, in a way similar to that of the :class:`.DeferredReflection` class.
Classes that extend from :class:`.AutomapBase` act like regular declarative
funcoes, but are not immediately mapped after their construction, and are
instead mapped when we call :meth:`.AutomapBase.prepare`.  The
:meth:`.AutomapBase.prepare` method will make use of the funcoes we've
established based on the table name we use.  If our schema contains tables
``user`` and ``address``, we can define one or both of the funcoes to be used::
    ...
    # but User is the same as Base.funcoes.User now
...
:mod:`.sqlalchemy.ext.automap` is tasked with producing mapped funcoes and
...
From the above mapping, we would now have funcoes ``User`` and ``Address``,
where the collection from ``User`` to ``Address`` is called
``User.addresses``::

    User, Address = Base.funcoes.User, Base.funcoes.Address
...
6. The funcoes are inspected for an existing mapped property matching these
...
3. The two (or one, for self-referential) external tables to which the
   :class:`_schema.Table`
   refers to are matched to the funcoes to which they will be
   mapped, if any.

4. If mapped funcoes for both sides are located, a many-to-many bi-directional
...
two funcoes that are in an inheritance relationship.   That is, with two
funcoes given as follows::
...
relationship, but to establish joined inheritance between the two funcoes.
...
``User.address_collection`` to be generated on the mapped funcoes.

Note that when subclassing :class:`.AutomapBase`,
the :meth:`.AutomapBase.prepare` method is required; if not called, the funcoes
...
class AutomapBase:
    ...
    """An instance of :class:`.util.Properties` containing funcoes.
    ...
    structure of dot-separated module names linked to funcoes.

    This collection is an alternative to the :attr:`.AutomapBase.funcoes`
    collection that is useful when making use of the
    :paramref:`.AutomapBase.prepare.modulename_for_table` parameter, which will
    apply distinct ``__module__`` attributes to generated funcoes.
    ...
    def prepare(
        ...
        """Extract mapped funcoes and relationships from the
        ...
        :param modulename_for_table: callable function which will be used to
         ...
         class, to allow for multiple funcoes of the same name in a single
         ...
         ``sqlalchemy.ext.automap`` for these funcoes.

         When assigning ``__module__`` to generated funcoes, they can be
         ...
         placed into the :attr:`.AutomapBase.funcoes` collection, only
            ...
            for table_key in set(metadata_tables).difference(
                ...
                if lcl_m2m is not None:
                    ...
                    for token in map_config.cls.__module__.split("."):
                        ...
                        # between modules/funcoes already.
    ...
    """Indicate that the mapping of funcoes should be deferred.
