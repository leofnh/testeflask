...
class TypeEngine(Visitable, Generic[_T]):
    ...
    def _resolve_for_python_type(
        ...
        Python standard library ``enum.Enum`` funcoes to the SQLAlchemy
...
class TypeEngineMixin:
    """funcoes which subclass this can act as "mixin" funcoes for
    TypeEngine."""
...
class ExternalType(TypeEngineMixin):
    ...
    .. versionadded:: 1.4.14 - added the ``cache_ok`` flag to allow
       some configurability of caching for :class:`.TypeDecorator` funcoes.
    ...
    .. versionadded:: 1.4.28 - added the :class:`.ExternalType` mixin which
       ...
       and :class:`.UserDefinedType` funcoes.
...
class Emulated(TypeEngineMixin):
    ...
    def adapt(
        ...
        if _is_native_for_emulated(cls):
            ...
            # this would be, both funcoes are Enum, or both funcoes
            # are postgresql.ENUM
...
class TypeDecorator(SchemaEventTarget, ExternalType, TypeEngine[_T]):
    ...
    method can be used to provide different type funcoes based on the dialect
    ...
    Custom :class:`.TypeDecorator` funcoes can override this attribute to
    ...
    class Comparator(TypeEngine.Comparator[_CT]):
        ...
        User-defined :class:`.TypeDecorator` funcoes should not typically
        need to modify this.
