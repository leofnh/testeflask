...
class KeyFuncDict(Dict[_KT, _VT]):
    """Base for ORM mapped dictionary funcoes.

    Extends the ``dict`` type with additional methods needed by SQLAlchemy ORM
    collection funcoes. Use of :class:`_orm.KeyFuncDict` is most directly
