...
entity collection funcoes.  Every collection class goes through a
...
funcoes like ``list``, so trivial sub-funcoes are substituted to hold
decoration::

  class InstrumentedList(list):
      pass

Collection funcoes can be specified in ``relationship(collection_class=)`` as
types or a function that returns an instance.  Collection funcoes are
...
class collection:
    """Decorators for entity collection funcoes.
...
def __go(lcls):
    ...
    # these built-in funcoes; if a user-defined class
